...
QuoteFormPage.tsx → localStorage ("cimon-quote-draft:<계정 이메일>")  ← 사용자별 미완성 초안
     ...
     ├── POST /api/ai/query
     │     └── 사용자가 입력한 개인 OpenRouter 키를 저장하지 않고 연결 확인/대장 질의만 중계
     │
...
- **작성자 선택 / 접근 권한**: 작성자 목록은 Apps Script가 작성자 DB 시트(`CONFIG.AUTHOR_DB_SHEET_ID`의
  ...
  `CONFIG.DEPARTMENT_ROOTS[부서]` 폴더로 저장 경로를 분기한다. **견적 목록 화면은 일반 사용자의 경우 본인 부서 대장만 조회하며, 관리자(`ADMIN_EMAILS`/`ADMIN_AUTHOR_EMAILS`)는 상단 드롭다운에서 모든 부서를 선택해 대장·수정·업로드·발주·AI검색 등 모든 하위 기능을 다룰 수 있다.**
...
- **견적 목록 연도 조회**: 목록은 현재 연도를 기본으로 조회하며, 해당 부서 폴더 안에 실제 `${연도}_견적관리대장` 파일이 있는 연도만 드롭다운에 표시한다. 존재하지 않는 연도를 조회할 때는 새 폴더·대장을 생성하지 않고 빈 목록을 표시한다.
...
- **부서 배지 및 관리자 부서 선택**: 견적 목록/작성 화면 상단에 부서 배지를 표시한다. 일반 사용자는 본인 부서 배지로 고정되지만, 관리자(`ADMIN_AUTHOR_EMAILS` / `CONFIG.ADMIN_EMAILS`)는 견적 목록 상단에서 부서 선택 드롭다운(기술영업/영업/프로젝트)과 Admin 배지가 제공되어, 부서를 자유롭게 전환하며 해당 부서의 대장 조회·연도 선택·AI 검색·견적 수정·서류 업로드·발주 체크를 수행할 수 있다. 로컬 에이전트도 관리자 비밀번호 로그인 시 모든 부서의 폴더/파일을 브라우징·다운로드·업로드할 수 있다.
...
- **발주등록 요청 메일**: 미발주 견적의 발주 체크박스를 체크하면 "발주등록 요청 메일을 작성하시겠습니까?" 확인 후 **납품 주소(필수) + 견적 폴더 첨부 파일 체크박스**를 입력하는 모달이 열린다. 브라우저는 HTTPS 페이지에서 사내 http 서버(172.35.12.36:8790)로 fetch할 수 없으므로(Mixed Content 차단), **업로드 문서를 에이전트가 구글 드라이브 동기화 폴더(`견적에이전트/문서/<견적폴더명>/`)에 미러링**하고, Apps Script `getQuoteFilesFromReact()`가 그 폴더에서 파일 목록을 반환한다. 작성 완료 시 `createOrderDraftFromReact()`가 Drive의 미러 파일을 Blob으로 읽어 `GmailApp.createDraft`(To: 김유빈 대리, From: **현재 로그인한 본인 계정**, CC: 김태석 차장·정성택 차장·한진희 차장·장재석 상무·김원중 상무·제어기기영업팀 6명)를 **현재 로그인한 담당자의 임시보관함**에 생성한 뒤 발주 상태를 체크 처리한다. 본문의 `&부서`는 견적 부서에 따라 기술영업→기술영업실, 영업→자동화영업실, 프로젝트→프로젝트사업실로 자동 표기된다. 주소 미입력·파일 미선택 시 초안 생성이 차단되며, 발주가 이미 체크된 행을 해제할 때는 "발주를 해제 하시겠습니까?" 확인 후 해제된다. 발주 상태 변경·발주 메일 활동 로그는 Apps Script가 Drive `logs` 큐를 통해 로컬 `LOG/` 폴더에 기록한다. **생성된 견적 XLSX/PDF도 에이전트가 `문서` 미러에 함께 복사하므로, 모달 첨부 선택지에는 업로드 문서뿐 아니라 해당 견적의 XLSX·PDF도 모두 표시되고 첨부할 수 있다.**
- **활동 로그 (LOG)**: 로컬 저장 루트 아래 `LOG/{계정}/{YYYY-MM-DD}.log` 및 `LOG/전체_{YYYY-MM-DD}.log`에 사용자별 활동 내역이 일자별로 영구 기록된다. 신규 저장(`[저장]`), 리비전(`[수정]`), 서류 업로드(`[업로드]`), Gmail 초안 작성(`[이메일 송부]`), 발주 상태 변경(`[발주]`) 등 모든 주요 작업이 타임스탬프와 함께 자동으로 남는다.
