...
- **발주등록 요청 메일**: 미발주 견적의 발주 체크박스를 체크하면 "발주등록 요청 메일을 작성하시겠습니까?" 확인 후 **납품 주소(필수) + 견적 폴더 첨부 파일 체크박스**를 입력하는 모달이 열린다. 작성 완료 시 로컬 에이전트의 `GET /api/files`로 견적 폴더 파일 목록을 받고, 선택 파일을 base64로 변환해 Apps Script `createOrderDraftFromReact()`가 `GmailApp.createDraft`(To: 김유빈 대리, CC: 조규광 이사·김태석 차장·정성택 차장·한진희 차장·jaesuk.chang·김원중 상무·제어기기영업팀 7명, 발주서·사업자등록증 등 첨부)를 **현재 로그인한 담당자의 임시보관함**에 생성한 뒤 발주 상태를 체크 처리한다. 주소 미입력·파일 미선택 시 초안 생성이 차단되며, 발주가 이미 체크된 행을 해제할 때는 "발주를 해제 하시겠습니까?" 확인 후 해제된다.
