...
- **발주등록 요청 메일**: 미발주 견적의 발주 체크박스를 체크하면 "발주등록 요청 메일을 작성하시겠습니까?" 확인 후 **로컬 에이전트(`172.35.12.36:8790`)의 발주등록 메일 페이지**가 새 탭으로 열린다. 부서 비밀번호로 로컬 견적 폴더의 생성 XLSX/PDF와 업로드 문서를 직접 선택하고 납품 주소를 입력한다. 선택 파일은 postMessage로 웹앱에 전달되고, `createOrderDraftFromReact()`가 현재 로그인한 본인 계정의 `GmailApp.createDraft`로 To: 김유빈 대리, CC: 김태석 차장·정성택 차장·한진희 차장·장재석 상무·김원중 상무·제어기기영업팀 6명에게 초안을 생성한 뒤 발주 상태를 체크 처리한다. 본문의 `&부서`는 견적 부서에 따라 기술영업→기술영업실, 영업→자동화영업실, 프로젝트→프로젝트사업실로 자동 표기된다. 주소 미입력·파일 미선택 시 초안 생성이 차단되며, 발주가 이미 체크된 행을 해제할 때는 "발주를 해제 하시겠습니까?" 확인 후 해제된다. 이 방식은 첨부 파일 선택을 위해 구글 드라이브 `문서` 폴더에 의존하지 않고 로컬 PC 파일을 직접 사용한다.
