...
- **AI 대장 검색**: 견적 목록의 `AI 검색` 패널에서 사용자가 개인 OpenRouter API 키를 연결 확인한 뒤 자연어 질문을 보낼 수 있다. 키는 브라우저 localStorage나 서버 저장소에 보관하지 않으며, 현재 선택한 연도의 대장 데이터 중 연락처·이메일을 제외한 요약 데이터만 요청에 포함한다.
