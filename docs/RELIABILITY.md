...
함께 수정한다. 현재 기본 열 순서는 `NO, 연도, 월, 일, 발주, 작성자, 견적번호, 업체명, 담당자, 연락처, 이메일,
제품군, 제품명, 금액, 비고, 파일링크`(A~P, 16열)이며, 레거시 15열 대장(F열=견적번호)도 동적 레이아웃(`getLedgerLayout_`)을
통해 완벽 호환된다. **E열(발주)은 사용자가 대장에서 직접
...
`assertLedgerLayout_()`로 E열 헤더에 "발주", F열 헤더에 "작성자"(또는 "견적번호")가 포함되는지 검증하고, 어긋나면
조용히 잘못된 열에 쓰는 대신 즉시 에러를 던진다 — **단, 연도가 바뀌어 `LEDGER_TEMPLATE_ID` 시트를
복사해 새 연도 대장을 만들 때는 템플릿 자체가 이미 이 16열 구성을 갖고 있어야 한다.** 템플릿에
발주 및 작성자 열이 없으면 새로 생성되는 모든 연도의 대장이 한 칸씩 밀려 저장되므로, 연말/연초에는 반드시
...
해당 견적 폴더(연도/부서/견적번호_업체명 조합으로 검증)에 발주서·사업자등록증·PPT 등 파일을 추가한다.
확장자는 화이트리스트가 아니라 블랙리스트로 검사하며(`.exe`/`.bat`/`.js`/`.html` 등 실행·스크립트
파일만 차단, 그 외 형식은 모두 허용), 파일당 최대 1GB까지 업로드할 수 있다. 부서 비밀번호 로그인이
필요하다. (base64 인코딩으로 전송하므로 실제 HTTP 요청 본문은 원본 크기의 약 1.33배가 된다 —
