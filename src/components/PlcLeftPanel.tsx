...
export default function PlcLeftPanel({
  ...
  return (
    <aside className="w-full flex-shrink-0">
      {onClose && (
        <div className="md:hidden flex items-center justify-between px-4 py-3 bg-[#3d5a73] border-b border-[#4a6880] mb-3 rounded-t-xl">
          <span className="text-sm font-semibold text-[#e0e0e0]">{t(UI.series)}</span>
          <button onClick={onClose} className="w-7 h-7 flex items-center justify-center text-[#888888] hover:text-white text-xl leading-none rounded-full hover:bg-[#4a6880]">×</button>
        </div>
      )}
      <div className="bg-[#3d5a73] rounded-lg p-3 mb-3">
            ...
            <button
              ...
              className={`flex-1 py-2 rounded-lg text-sm font-semibold transition-colors ${
                plcSeries === s
                  ...
                  : 'bg-[#4a6880] text-[#c8c5c0] hover:bg-[#333333]'
              }`}
            >
      ...
      <div className="bg-[#3d5a73] rounded-lg overflow-hidden">
        <div className="py-1">
          {tree.map((group) => {
            ...
            return (
              <div key={group.id}>
                <button
                  ...
                  className={`w-full flex items-center justify-between px-4 py-2.5 text-left transition-colors hover:bg-[#4a6880] ${
                      ...
                      return (
                        <button
                          ...
                          className={`w-full text-left pl-7 pr-4 py-2 text-sm transition-colors ${
                            active
                              ? 'bg-[#4a6880] text-white font-medium border-l-2 border-white'
                              : 'text-[#888888] hover:bg-[#4a6880] hover:text-[#e0e0e0] border-l-2 border-transparent'
                          }`}
                        >
                          {leafLabel}
                        </button>
